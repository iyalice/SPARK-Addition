...
for (cnt in 1:2) {
  ...
  # pr数据的计算
  pr_data <- append(pr_data, list(pr.curve(scores.class0 = pval, weights.class0 = labels, curve=T)))
}

tempdata1 <- pr_data[[1]]$curve
tempdata2 <- pr_data[[2]]$curve

# recall1 <- pr_data[[1]]$curve[,1]
# precision1 <- pr_data[[1]]$curve[,2]
# recall2 <- pr_data[[2]]$curve[,1]
# precision2 <- pr_data[[2]]$curve[,2]


# pr曲线的绘制
# png(paste0(PRplotname, ipow, ".png"), width = 8*600, height = 8*600, res = 600)
# png(paste0(PRplotname, ipow, ".png"), width = 8*600, height = 8*600, res = 600)

plot(pr_data[[1]], col = "blue", lty = 1, lwd = 2, ann = FALSE)
# plot(recall1, precision1, pch = 1)
title(main = paste0("PR - Curve", " (Pattern ", ptn[ipt-1], ")"), cex.main = 1.5)
# plot(pr_data[[2]], col = "red", lty = 1, lwd = 2, add = TRUE)
# plot(recall2, precision2, pch = 1)
legend("bottomleft", legend = c(MethodName[1], MethodName[5]), col = c("blue", "red"), lty = c(1, 1), lwd = 2)

text(0.45, 0.03, paste(MethodName[1], " AUC:", round(pr_data[[1]]$auc.integral, 7)), col = "blue")
text(0.5, 0, paste(MethodName[5], " AUC:", round(pr_data[[2]]$auc.integral, 7)), col = "red")
