...
for (ipt in 2:4) {
  ...
  temp <- list(ggplot(data=FDR_power, aes(x=FDR, y=Power, group=method, color=method)) + 
                 geom_line(size = 0.5) + theme_bw() + coord_cartesian(ylim=c(0,1)) + 
                 labs(title = paste("Pattern", ptn[ipt-1])) +
                 theme(plot.title = element_text(family = "Lato", size=12, hjust=0.5),
...
# 最后一张图添加图例
plot_list[[3]] <- plot_list[[3]] + theme(plot.title = element_text(family = "Lato", size=12, hjust=0.5),
                                         legend.position = c(0.5, 0.3),#plot内位置
                                         legend.text = element_text(family = "Lato", size = 8), # 设置图例文本大小
...
ggsave(filename = plotname_png, FPplot, dpi = 600)
print(FPplot)
